	...
	public CaseDetails fetchCaseDetails(String caseType, String caseNumber, String filingYear) {
		...
		while (true) {
			try {
				...
				// 3️⃣ Parse basic info
				String filingDate = resultDoc.select("td:contains(Date of Filing :) + td").text().trim();
				String nextHearingDate = resultDoc.select("td:matches((?i)Date of Next Hearing :) + td").text().trim();
				...
				// ✅ 4️⃣ Check if invalid case (no filing date and no parties)
				if (filingDate.isEmpty() && (parties.equals("Not Available") || parties.isEmpty())) {
					...
					errorResponse.setOrderPdfs(""); // ✅ Empty string for invalid case
					return errorResponse; // 🚨 Don't save to DB
				}

				// ✅ 5️⃣ Default empty string for PDFs
				String orderPdfs = "";

				// 6️⃣ Hidden fields for Listing Details
					...
					try {
						...
						if (!pdfLinks.isEmpty()) {
							orderPdfs = pdfLinks.stream().map(link -> link.attr("abs:href").trim()).distinct()
									.collect(Collectors.joining(",")); // ✅ Store as comma-separated string
						}
				...
				details.setOrderPdfs(orderPdfs); // ✅ Save as String
				...
				if (attempt >= maxRetries) {
					...
					// ✅ Return safe error response instead of raw 500
					CaseDetails errorResponse = new CaseDetails();
					errorResponse.setCaseType(caseType);
					errorResponse.setCaseNumber(caseNumber);
					errorResponse.setFilingYear(filingYear);
					errorResponse.setParties("Court site error or timed out");
					errorResponse.setOrderPdfs(""); // ✅ Empty string
